...
> 状态：IMPLEMENTED BASELINE；两个正式 Web 工程、两个小程序平台适配层和共享 API/会话层已落地
> 更新时间：2026-09-02

React、TypeScript、Vite、pnpm workspace、共享 contracts/API client/auth/task-domain/ui、两个 Web 应用壳层和两个原生小程序应用壳层已经落地。Node.js v24.20.0、npm 11.19.0、pnpm 9.15.0 已配置，依赖已安装，前端 lint/typecheck/unit/build 已通过；浏览器联调和两个小程序仍需真实 AppID、通信域名和平台构建链。
...
| 员工端运行时 | 个人微信小程序 + 企业微信小程序 | 两个独立发布的原生客户端分别适配 `wx.login` 与企业微信原生登录 API；共享纯 TypeScript 契约和员工业务模型 |
...
员工端首期采用两个移动端优先的小程序：个人微信 `employee-miniapp` 与企业微信 `employee-wecom-miniapp`。两者共享任务模型和 Edge API，但独立发布、独立客户端会话标识和平台登录适配；同时保留正式可用的 `employee-web` 网页版，供直接浏览器访问或小程序入口不可用时使用。管理端保持桌面优先的响应式 Web。可靠的 received/start/complete 写入仍必须经过 Edge 持久化 Command，不能依赖小程序或浏览器本地缓存/离线队列；received 只代表已收到通知，不代表员工同意。

`apps/employee-miniapp` 和 `apps/employee-wecom-miniapp` 作为两个独立客户端创建。它们复用 `packages/contracts`、`packages/task-domain`、错误归一化和 API 契约；平台登录、存储、通信域名和客户端标识分别隔离，共享业务域不得直接依赖 `wx.*`。
...
│   │       └── pages/
│   ├── employee-miniapp/          # 个人微信员工小程序
│   │   └── src/
│   │       ├── app/              # 启动上下文、Session、平台适配
│   │       ├── features/tasks/   # Projection、received/start/complete
│   │       └── pages/
│   ├── employee-wecom-miniapp/    # 企业微信员工小程序
│   │   └── src/
│   │       ├── app/              # 企业微信启动上下文、Session、平台适配
│   │       └── platform/         # wx.qy.login、wx.request、存储适配
...
- 关键操作不等待动画；Confirm、Cancel、received、start、complete、Retry 必须立即发起请求并显示真实 pending/syncing 状态。
