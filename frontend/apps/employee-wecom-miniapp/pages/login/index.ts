import { passwordLoginAndBind, quickLogin } from "../../src/app/provider-login";
...
Page({
  ...
  async quickLogin() {
    ...
    try {
      ...
      this.setData({ error: error instanceof Error ? error.message : "企业微信快捷登录失败，请先使用工号密码绑定" });
    } finally {
  ...
  async passwordLogin() {
    if (!this.data.employeeNo || !this.data.password) {
      this.setData({ error: "请输入项目员工工号和密码" });
      return;
    }
  ...
  async passwordOnlyLogin() {
    this.setData({ passwordLoading: true, error: "" });
    try {
      await miniappSession.passwordLogin({ employee_no: this.data.employeeNo, password: this.data.password, client: "employee-wecom-miniapp" });
      wx.reLaunch({ url: "/pages/tasks/index" });
    } catch (error) {
      this.setData({ error: error instanceof Error ? error.message : "登录失败，请稍后重试" });
    } finally {
