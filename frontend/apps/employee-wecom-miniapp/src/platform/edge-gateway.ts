...
// WeChat miniapps cannot use the browser WebSocket object, but the delivery
// contract is the same: a short-lived ticket authenticates the socket and a
// task_changed message only asks the page to pull a fresh Edge snapshot.
export class MiniappRealtimeClient {
  ...
  private toWebSocketURL(): string {
    return `${this.baseUrl.replace(/^http:/, "ws:").replace(/^https:/, "wss:").replace(/\/$/, "")}/api/v1/ws/native`;
  }
...
export class MiniappSessionStore implements SessionStore {
  constructor(private readonly key = "flight.edge.employee-wecom-miniapp.session") {}
