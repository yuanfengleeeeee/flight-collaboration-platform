...
import { canComplete, canReceive, canStart, changeActionsForTaskStatus, commandStatusLabel, createClientID, projectionStatus, suggestedChangeActionForExceptionCategory, taskChangeActionLabel, taskReceiptLabel, taskStatusLabel } from "@flight/task-domain";
...
type EmployeeTaskFilter = "all" | "pending_receipt" | "received" | TaskStatus;
type EmployeeAllowedChangeActions = TaskChangeAction[] & { includes(searchElement: TaskChangeAction | "", fromIndex?: number): boolean };

function EmployeeTaskListPage(): JSX.Element {
  const [items, setItems] = useState<EdgeTaskProjection[]>([]);
  const [projection, setProjection] = useState<{ lagState: "known" | "unknown"; lagSeconds: number; snapshotAt: string; revision: number }>();
  const [filter, setFilter] = useState<EmployeeTaskFilter>("all");
  ...
  useEffect(() => { const controller = new AbortController(); setLoading(true); setError(undefined); edgeApi.listTasks(controller.signal).then((response) => { setItems(response.items); setProjection({ lagState: response.projection_lag_state, lagSeconds: response.projection_lag_seconds, snapshotAt: response.snapshot_at, revision: response.projection_revision }); }).catch((value: unknown) => { if (!controller.signal.aborted) setError(toErrorMessage(value, "无法读取你的任务，请稍后重试。")); }).finally(() => { if (!controller.signal.aborted) setLoading(false); }); return () => controller.abort(); }, [reloadKey]);
  const visible = filter === "all" ? items : items.filter((item) => matchesEmployeeTaskFilter(item, filter));
  return <div className="employee-page"><div className="employee-page-heading"><div><p className="eyebrow">TODAY / MY WORK</p><h1>我的任务</h1><p>只展示 Edge 返回给当前员工的完整快照；“我已收到”只是收件回执，不代表同意任务。</p></div><button className="refresh-button" onClick={() => setReloadKey((value) => value + 1)}>刷新</button></div>{projection && <div className={`projection-health projection-${projection.lagState}`} role="status">快照：{projection.lagState === "known" ? `延迟 ${projection.lagSeconds} 秒` : "同步延迟未知"} · 版本 {projection.revision} · {formatDate(projection.snapshotAt)}</div>}<div className="employee-counts"><button className={filter === "all" ? "is-active" : ""} onClick={() => setFilter("all")}>全部 <strong>{items.length}</strong></button><button className={filter === "pending_receipt" ? "is-active" : ""} onClick={() => setFilter("pending_receipt")}>待确认收件 <strong>{items.filter((item) => matchesEmployeeTaskFilter(item, "pending_receipt")).length}</strong></button><button className={filter === "received" ? "is-active" : ""} onClick={() => setFilter("received")}>已收到待开始 <strong>{items.filter((item) => matchesEmployeeTaskFilter(item, "received")).length}</strong></button><button className={filter === "in_progress" ? "is-active" : ""} onClick={() => setFilter("in_progress")}>进行中 <strong>{items.filter((item) => projectionStatus(item) === "in_progress").length}</strong></button><button className={filter === "paused" ? "is-active" : ""} onClick={() => setFilter("paused")}>已暂停 <strong>{items.filter((item) => projectionStatus(item) === "paused").length}</strong></button><button className={filter === "completed" ? "is-active" : ""} onClick={() => setFilter("completed")}>已完成 <strong>{items.filter((item) => projectionStatus(item) === "completed").length}</strong></button></div>{loading && <div className="employee-state">正在读取 Edge Projection…</div>}{error && <div className="employee-state"><Notice tone="error">{error}<button className="employee-secondary" onClick={() => setReloadKey((value) => value + 1)}>重新加载</button></Notice></div>}{!loading && !error && visible.length === 0 && <div className="employee-state"><span className="empty-symbol">∅</span><h2>当前没有符合条件的任务</h2><p>任务由系统自动派发，并通过 Edge Projection 出现在这里。</p></div>}{!loading && !error && visible.length > 0 && <div className="employee-task-list">{visible.map((task) => <EmployeeTaskCard key={task.public_id} task={task} />)}</div>}</div>;
}

function matchesEmployeeTaskFilter(task: EdgeTaskProjection, filter: EmployeeTaskFilter): boolean {
  const status = projectionStatus(task);
  if (filter === "pending_receipt") return status === "assigned" && task.receipt_status !== "received";
  if (filter === "received") return status === "assigned" && task.receipt_status === "received";
...
function EmployeeTaskCard({ task }: { task: EdgeTaskProjection }): JSX.Element {
  ...
  return <Link className={`employee-task-card task-status-${status}`} to={`/tasks/${task.public_id}`}><div className="task-card-top"><span className="flight-number">{task.flight_display_no || "—"}</span><StatusBadge status={status} label={taskStatusLabel[status]} /></div><h2>{task.task_name}</h2><p>{task.area_name || "未提供区域"} · 计划 {formatDate(task.planned_at)}</p><div className="task-card-bottom"><span>{taskReceiptLabel(status, receiptStatus)} · {task.message || "暂无任务说明"}</span><strong>查看详情 →</strong></div></Link>;
}
...
function EmployeeTaskDetailPage(): JSX.Element {
  ...
  useEffect(() => { if (!command?.id) return; let stopped = false; let timer: number | undefined; let failures = 0; const poll = async (): Promise<void> => { try { const response = await edgeApi.commandStatus(command.id); if (stopped) return; failures = 0; const nextStatus = response.data.status; setCommand((current) => current ? { ...current, status: nextStatus } : current); const stored = taskCommandStore.read(taskPublicID); if (stored?.id === command.id) taskCommandStore.write({ ...stored, status: nextStatus }); if (nextStatus === "confirmed" || nextStatus === "failed") { taskCommandStore.clear(command.id); setDraftCommand(undefined); if (nextStatus === "failed") setCommandError(response.data.error_code ?? "command_failed"); if (nextStatus === "confirmed") load(); return; } timer = window.setTimeout(() => void poll(), 2000); } catch (value) { if (!stopped) { failures += 1; setCommandError(toErrorMessage(value, "暂时无法查询操作同步状态，系统会自动重试。")); timer = window.setTimeout(() => void poll(), Math.min(10_000, 2_000 * (2 ** Math.min(failures, 3)))); } } }; void poll(); return () => { stopped = true; if (timer) window.clearTimeout(timer); }; }, [command?.id, load, taskPublicID]);
...
function EmployeeExceptionPage(): JSX.Element {
  ...
  const selectedTask = tasks.find((task) => task.public_id === taskID);
  const availableChangeActions = useMemo(() => (selectedTask ? changeActionsForTaskStatus(projectionStatus(selectedTask)) : []) as EmployeeAllowedChangeActions, [selectedTask]);
  useEffect(() => {
    setChangeAction((current) => current && !availableChangeActions.includes(current) ? availableChangeActions[0] ?? "" : current);
    if (!availableChangeActions.includes("reschedule")) setTargetPlannedAt("");
  }, [availableChangeActions]);

  useEffect(() => {
    const controller = new AbortController();
    edgeApi.listTasks(controller.signal).then((response) => {
      const available = response.items.filter((task) => Boolean(task.assignment_public_id) && projectionStatus(task) !== "cancelled" && projectionStatus(task) !== "completed");
      setTasks(available);
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const allowedActions = task ? changeActionsForTaskStatus(projectionStatus(task)) : [];
    if (!task?.assignment_public_id || !description.trim() || (changeAction !== "" && !allowedActions.includes(changeAction as TaskChangeAction)) || (changeAction === "reschedule" && !targetPlannedAt)) return;
  ...
  return <div className="employee-page"><div className="employee-page-heading"><div><p className="eyebrow">EMPLOYEE / EXCEPTIONS</p><h1>异常反馈</h1><p>异常会以持久化 Command 送达 Core，由 Core 校验任务归属、版本和权限后写入事实；提交是受控申请，不会直接拒绝或修改任务。</p></div></div>{loading && <div className="employee-state">正在读取任务…</div>}{error && <Notice tone="error">{error}</Notice>}{notice && <Notice tone="success">{notice}</Notice>}{!loading && tasks.length === 0 && <div className="employee-state empty-page"><span className="empty-symbol">∅</span><h2>当前没有可反馈任务</h2><p>需要先有属于你的任务 Projection。</p></div>}{!loading && tasks.length > 0 && <form className="employee-detail-card employee-exception-form" onSubmit={submit}><label>关联任务<select value={taskID} onChange={(event) => setTaskID(event.target.value)}>{tasks.map((task) => <option key={task.public_id} value={task.public_id}>{task.flight_display_no || task.public_id} · {task.task_name}</option>)}</select></label>{selectedTask && <p className="form-hint">当前状态：{taskStatusLabel[projectionStatus(selectedTask)]}。可申请动作由 Core 状态机约束。</p>}<label>异常类别<select value={category} onChange={(event) => { const next = event.target.value; const suggested = suggestedChangeActionForExceptionCategory(next) ?? ""; const selected = availableChangeActions.includes(suggested) ? suggested : ""; setCategory(next); setChangeAction(selected); if (selected !== "reschedule") setTargetPlannedAt(""); }} required><option value="保障冲突（申请重新预分配）">保障冲突（申请重新预分配）</option><option value="航班延误（申请调整任务）">航班延误（申请调整任务）</option><option value="航班取消（申请取消任务）">航班取消（申请取消任务）</option><option value="突发事件（申请值班经理处置）">突发事件（申请值班经理处置）</option><option value="其他现场异常">其他现场异常</option></select></label><label>申请动作（可选，需经理审批）<select value={changeAction} onChange={(event) => { const next = event.target.value as TaskChangeAction | ""; setChangeAction(next); if (next !== "reschedule") setTargetPlannedAt(""); }}><option value="">仅上报，不申请变更</option>{availableChangeActions.map((action) => <option key={action} value={action}>{taskChangeActionLabel[action]}</option>)}</select></label>{changeAction === "reschedule" && <label>建议的新计划时间<input type="datetime-local" value={targetPlannedAt} onChange={(event) => setTargetPlannedAt(event.target.value)} required /></label>}<label>严重程度<select value={severity} onChange={(event) => setSeverity(event.target.value as ExceptionSeverity)}><option value="low">低</option><option value="medium">中</option><option value="high">高</option><option value="critical">紧急</option></select></label><label>现场描述<textarea value={description} onChange={(event) => setDescription(event.target.value)} maxLength={1024} rows={6} placeholder="描述冲突航班、影响范围、现场位置和需要的协助" required /></label><button className="employee-primary" type="submit" disabled={busy || !description.trim() || (changeAction === "reschedule" && !targetPlannedAt)}>{busy ? "正在提交…" : "提交受控申请"}</button></form>}</div>;
}
