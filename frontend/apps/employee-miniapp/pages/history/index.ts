...
Page({
  data: { items: [] as EdgeTaskProjection[], loading: true, error: "", page: 1, pageSize: 20, total: 0, realtimeState: "idle" },

  onShow() {
    this.loadHistory(this.data.page);
    miniappRealtime.start(() => this.loadHistory(this.data.page), (state) => this.setData({ realtimeState: state }), () => wx.reLaunch({ url: "/pages/login/index" }));
  },
